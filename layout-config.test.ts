...
describe("normaliseLayoutConfig", () => {
  ...
  it("uses the Claude-like block defaults", () => {
    const out = normaliseLayoutConfig(undefined);
    ...
    expect(out.enabled.cost).toBe(false);
    expect(out.enabled.tokens).toBe(false);
    expect(out.enabled.rate).toBe(false);
  });
});
...
describe("normaliseLayoutConfig flags", () => {
  it("treats missing enabled keys as the fork defaults", () => {
    const out = normaliseLayoutConfig({
      // Only one key supplied — every other known id should keep the default.
      enabled: { git: false } as Record<string, boolean>,
    });
    expect(out.enabled.git).toBe(false);
    for (const id of KNOWN_BLOCK_IDS) {
      if (id === "git") continue;
      expect(out.enabled[id]).toBe(DEFAULT_LAYOUT_CONFIG.enabled[id]);
    }
  });
...
describe("clampSeparator", () => {
  ...
  it("preserves the canonical defaults exposed on DEFAULT_LAYOUT_CONFIG", () => {
    ...
    expect(DEFAULT_LAYOUT_CONFIG.order).toEqual([
      "path",
      "git",
      "model",
      "context",
    ...
    expect(DEFAULT_LAYOUT_CONFIG.enabled).toMatchObject({
      model: true,
      path: true,
      git: true,
      context: true,
...
describe("cloneDefaultLayout", () => {
  it("returns a deep mutable copy", () => {
    ...
    expect(b.order).toEqual([...DEFAULT_LAYOUT_CONFIG.order]);
