/**
 * @andreagrandi/pi-statusline — persistent config for the events tracker.
 *
 * Stored at `~/.pi/agent/andreagrandi-pi-statusline/events.json`.
 * Legacy `~/.pi/agent/savagelands-net-pi-statusline/events.json` data is migrated
 * on first load when no andreagrandi config exists.
 *
...
import { type IconSet, isIconSet } from "./icons.ts";
import {
	cloneDefaultLayout,
...
export interface DisplayConfig {
	/** Master switch for the statusline widget itself. */
	 ...
	 *  `"emoji"` for a Claude-like, portable statusline. */
	iconSet: IconSet;
}
...
/** Resolve `~/.pi/agent/andreagrandi-pi-statusline/events.json`. */
